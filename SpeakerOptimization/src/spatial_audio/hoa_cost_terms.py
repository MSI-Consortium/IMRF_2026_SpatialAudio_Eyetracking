"""
hoa_cost_terms.py  (LEGACY -- retained for reference only)

This module contains a surrogate gain model and HOA-style cost terms that
were used before the project switched to real VBAP triangulation. The
surrogate_directional_gains() function does NOT match true VBAP gains.

The useful metrics from this module -- rV magnitude and rE magnitude --
have been moved into src/spatial_audio/vbap.py::evaluate_vbap_quality()
where they operate on real VBAP gains instead of surrogates.
...
from ..geometry.geometry_utils import clamp01, safe_unit, speaker_dirs_and_distances
...
def score_hoa_for_listener(
    ...
    weight_keys = ["rV_ang", "rV_mag", "rE_ang", "rE_mag", "match", "gain"]

    num = (
        weights["rV_ang"] * e_rV_ang
    ...
    den = sum(weights[k] for k in weight_keys if k in weights)
