...
def plot_multiple_layouts(
        feasible_boxes, forbidden_boxes, listener_positions, layouts, scores
):
    """
    Plot multiple layouts (initial, 33%, 66%, 100%) together, including their corresponding scores.
    ...
    - layouts: List of layouts (speaker positions) to plot.
    - scores: List of corresponding scores for each layout.
    """
    ...
    # Plot listener positions (yellow stars)
    ...
    # Plot each layout (black dots)
    for i, (layout, score) in enumerate(zip(layouts, scores)):
        ax.scatter(layout[:, 0], layout[:, 1], layout[:, 2],
                   marker='o', s=50, label=f"Layout {int(i * 33)}% - Score: {score:.3f}")

    # Set labels and title
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
    ax.set_zlabel("Z")
    ax.set_title("Multiple Room Layouts at Various Stages")
